	...
	*/
	...
	function katalog()
	{
		...
		$x['menu']			= 'katalog';
					...
					$config['base_url'] 	= base_url('home/katalog').'/'.$this->uri->segment(3);
					$config['per_page'] 	= $limit;
					$config['uri_segment']	= 4;
					$this->pagination->initialize($config);
						
					if($this->uri->segment(3)){
						$config['total_rows'] 	= $this->db->like('auto',$this->uri->segment(3))->get('t_produk')->num_rows();
						$offset 				= $this->uri->segment(4);
						$this->pagination->initialize($config);
						(is_numeric($this->uri->segment(3))) ? $this->db->where('a.id_merk',$this->uri->segment(3)) : $this->db->like('a.auto',$this->uri->segment(3)) ;
						$sql 	 	 		= $this->db->DISTINCT()
									...
									//  		->or_where('a.id_merk',$this->uri->segment(4))
									   		->limit($limit,$offset)
									   		->get()->result_array();
				/*	}elseif(isset($_GET['m'])){
					
						$config['total_rows'] 	= 10;//$this->db->get('t_produk')->num_rows();
						$this->pagination->initialize($config);
						$sql 	 	 		= $this->db->DISTINCT()
									   		->select('a.*, a.auto as auto, b.nama_merk, c.kategori')
									   		->from('t_produk a')
				...
				*/	}else{
						...
						$this->pagination->initialize($config);
						$sql 	= $this->db->get('t_kategori');
					}
		$view 				= ($this->uri->segment(3)) ? 'katalog_produk' : 'katalog';
		$x['panel_tengah']	= $this->load->view($view,array('sql'=>$sql),true);
		$x['error']			= '';
		$x['panel_kiri'] 	= '';
		$x['panel_kanan']	= '';
		$this->load->view('mainpage',$x);
	...
	function testimoni()
	{
		...
		switch ($this->input->post('submit')) {
			case 'simpan':
				...
				$d = $this->db->where('pesan',$c)->get('t_testimoni')->num_rows();
				if($a == ''){
					...
					$error = 'Testimoni sudah disampaikan sebelumnya';
					$x['panel_tengah']	= $this->load->view('testimoni',array('error'=>$error,'sql2'=>$sql2),true);
				}else{
